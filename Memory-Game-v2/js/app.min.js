...
setTimeout(_=>{deck.addEventListener('pointerdown',flip);document.addEventListener('keydown',shortcuts)},0)},750)};dropdown.addEventListener('click',toggleMenu);dpMenu.addEventListener('click',getFaction);start.addEventListener('pointerdown',_=>faction?modal.close(factions):0);deck.addEventListener('pointerdown',flip);shuffleBtn.addEventListener('pointerdown',shuffle);facBtn.addEventListener('pointerdown',_=>modal.open(factions));cog.addEventListener('pointerdown',_=>modal.open(settings));close[0].addEventListener('pointerdown',_=>modal.close(end));document.addEventListener('keydown',shortcuts);document.addEventListener('pointerdown',e=>{e.target.classList=="settings"?modal.close(settings):e.target.classList=="end"?modal.close(end):0});document.querySelector('#slide').addEventListener('pointerdown',toggleControls)