...
const makeArray = e => {
  ...
  solution.innerHTML = ''; // clean the existing results, if there is any
  pyramid.innerHTML = '';
  ...
  return add(line.map(e => e.map((e, i) => e = Math.floor(Math.random() * 10)))); // assign a random value (from 1 to 9) to each sub element
}
